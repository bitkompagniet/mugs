const nodemailer = require('nodemailer');
const handlebars = require('handlebars');
const fs = require('fs');
const path = require('path');
const _ = require('lodash');


function mailController (user, smtp, templatePath, emailData) {
	return new Promise(function(resolve, reject) {
		fs.readFile(path.join(__dirname, templatePath), 'utf8', function(err, data) { // eslint-disable-line 
			if (err) {
				return reject(err);
			}

			if (user.confirmed != null) {
				return reject(new Error('User already confirmed'));
			}

			const source = data;
			const template = handlebars.compile(source);

			const transporter = nodemailer.createTransport(smtp);

			const mailConfig = _.merge({}, user, emailData);

			const mailOptions = {
				from: '"mugs" <foo@blurdybloop.com>', // sender address
				to: mailConfig.email, // list of receivers
				subject: 'Confirmation from mugs', // Subject line
				text: '', // plaintext body
				html: template(mailConfig),
			};

			transporter.sendMail(mailOptions, function(error, info) {
				if (error) {
					return reject(error);
				}
				return resolve(info);
			});
		});
	});
}

module.exports = {
	recover: (user, config, resetPasswordToken) => {
		const templatePath = '/email_templates/recoverpass.hbs';
		const emailData = {
			confirmationLink: config.confirmationLink + resetPasswordToken,
			appName: config.appName,
			logoLink: config.logoLink,
		};
		return mailController(user, config.smtp, templatePath, emailData);
	},
	confirmation: (user, config, confirmationToken) => {
		const templatePath = '/email_templates/confirmation.hbs';
		const emailData = {
			confirmationLink: config.confirmationLink + confirmationToken,
			appName: config.appName,
			logoLink: config.logoLink,
		};
		return mailController(user, config.smtp, templatePath, emailData);
	},
};

